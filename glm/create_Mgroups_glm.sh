...
echo "-----------------------------------------------------------------"
# ==================================================================================
# usage:	. ./path/create_Mgroups_glm.sh /path/proj_dir -model path/templ.fsf -odp /path/output_dir -isubjf /path/input_subjects_file.txt -nuisids "1,2,3" -covids "4,5,6" -nomodelcreate
# ==================================================================================

# ==================================================================================
# create a FSF file starting from a template and filling in:
# - subjects number within each group
...
# uncomment to debug...so create always the same filename
#RND_NAME=""
...
#echo -----------DOUBLE_MEAN_CONTRASTS: $DOUBLE_MEAN_CONTRASTS
#echo -----------tot_cont: $tot_cont

OUTPUT_GLM_FSF=$OUTPUT_ROOT_DIR/group_comp$RND_NAME.fsf
cp $INPUT_TEMPL_GLM_FSF $OUTPUT_GLM_FSF
# DEBUG
#echo "---------------- S U M M A R Y -------------------------------------------------------------------------------"
#echo "creating Ngroups glm file  with the following parameters:"
#echo "NUM_GROUPS : $NUM_GROUPS"
...
if [ $NUM_GROUPS -eq 1 ]; then
	...
	if [ $NUM_GROUPS -eq 2 ]; then
		...
		con_id=$con_id+1
		echo "set fmri(conpic_real.$con_id) 1" >> $OUTPUT_GLM_FSF
		echo "set fmri(conname_real.$con_id) \"group1 > group3\"" >> $OUTPUT_GLM_FSF
		echo "set fmri(con_real$con_id.1) 1" >> $OUTPUT_GLM_FSF				
		echo "set fmri(con_real$con_id.3) -1" >> $OUTPUT_GLM_FSF				

		con_id=$con_id+1
		echo "set fmri(conpic_real.$con_id) 1" >> $OUTPUT_GLM_FSF
		echo "set fmri(conname_real.$con_id) \"group3 > group1\"" >> $OUTPUT_GLM_FSF
		echo "set fmri(con_real$con_id.1) -1" >> $OUTPUT_GLM_FSF				
		echo "set fmri(con_real$con_id.3) 1" >> $OUTPUT_GLM_FSF	
		...
		con_id=$con_id+1
		echo "set fmri(conpic_real.$con_id) 1" >> $OUTPUT_GLM_FSF
		echo "set fmri(conname_real.$con_id) \"group2 > group3\"" >> $OUTPUT_GLM_FSF
		echo "set fmri(con_real$con_id.2) 1" >> $OUTPUT_GLM_FSF				
		echo "set fmri(con_real$con_id.3) -1" >> $OUTPUT_GLM_FSF				

		con_id=$con_id+1
		echo "set fmri(conpic_real.$con_id) 1" >> $OUTPUT_GLM_FSF
		echo "set fmri(conname_real.$con_id) \"group3 > group2\"" >> $OUTPUT_GLM_FSF
		echo "set fmri(con_real$con_id.2) -1" >> $OUTPUT_GLM_FSF				
		echo "set fmri(con_real$con_id.3) 1" >> $OUTPUT_GLM_FSF	

	else 
		echo "contrasts to be implemented .... exiting"
		exit
	fi
fi
