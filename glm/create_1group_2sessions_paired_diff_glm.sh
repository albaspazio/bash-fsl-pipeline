...
do
	...
	sid=$(echo "scale=0;($ev-1)+$NUM_SUBJ" | bc)
	echo "set fmri(evg$sid.$ev) 1" >> $OUTPUT_GLM_FSF
done
...
echo "set fmri(conname_real.1) \"sess1>sess2\"" >> $OUTPUT_GLM_FSF
echo "set fmri(conpic_real.2) 1" >> $OUTPUT_GLM_FSF
echo "set fmri(conname_real.2) \"sess2>sess1\"" >> $OUTPUT_GLM_FSF		
		
echo "set fmri(con_real1.1) 1" >> $OUTPUT_GLM_FSF
echo "set fmri(con_real2.1) -1" >> $OUTPUT_GLM_FSF

echo "#====================== reset other contrast values" >> $OUTPUT_GLM_FSF
for (( ev=2; ev<=$tot_EV; ev++ ))
do
