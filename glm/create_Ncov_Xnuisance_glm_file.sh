...
do
  case "$1" in
  	...
  	-model)						INPUT_TEMPL_GLM_FSF=$2;shift;;
  							
    -odp) 						OUTPUT_ROOT_DIR=$2;shift;;
												
    -ofn) 						OUTPUT_ROOT_FILE_NAME=$2;shift;;												
								
	-isubjf)					SUBJECTS_FILE=$2;shift;;
								
	-groupslastids)		        GROUPSLASTSIDS=$2;shift;;	
																
	-nuisids)					NUISANCE_COLUMNS=$2;shift;;									
		
	-covids)					COVARIATE_COLUMNS=$2;shift;;	
												
	-rndname) 					RND_NAME=$2;shift;;	
...
#		  	echo $NUM_NUIS :: ${ARR_NUISANCE_COLUMNS[@]}    
    	...
    	OUTPUT_GLM_FSF=$OUTPUT_ROOT_DIR/$OUT_FSF_NAME$RND_NAME
    	continue
 	fi
...
if [ -z $GROUPSLASTSIDS ]; then 
	...
	grp_str="_"$NUM_GROUPS"groups"
fi

#---------------------------------------------------------------------------------------------------------------------
# define OUTPUT_GLM_FSF name
#---------------------------------------------------------------------------------------------------------------------
OUTPUT_GLM_FSF=${OUTPUT_GLM_FSF}${grp_str}.fsf
...
elif [ $NUM_GROUPS -eq 2 ]; then
	if [ $NUM_COVS -gt 0 ]; then
		NUM_MEAN_CONTRASTS=0
		tot_cont=2*$NUM_COVS*$NUM_GROUPS+2*$NUM_COVS
	else # $NUM_COVS = 0
		tot_cont=$(echo "scale=0;$NUM_GROUPS*($NUM_GROUPS-1) + $NUM_MEAN_CONTRASTS*$NUM_GROUPS" | bc)
	fi
else
	echo "3 groups are not supperted.....exiting"
	exit
fi
#if [ $NUM_GROUPS -eq 1 ]; then
...
#	if [ $NUM_COVS -gt 0 ]; then
#	
#		if [ $SKIP_MEAN_CONTRASTS -eq 1 ]; then
#			tot_cont=2*$NUM_COVS			# 2 x M cov 
#		else
#			if [ $DOUBLE_MEAN_CONTRASTS -eq 1 ]; then
#				tot_cont=2*$NUM_COVS + 2 			# 2 x M cov + 
#			else
#				tot_cont=2*$NUM_COVS + 1 			# 2 x M cov + 
#			fi
#		fi
#	else # $NUM_GROUPS = 1: $NUM_COVS>0
#	
#		if [ $DOUBLE_MEAN_CONTRASTS -eq 1 ]; then
#			tot_cont=2
#		else
...
#	if [ $NUM_COVS -gt 0 ]; then
#		if [ $SKIP_MEAN_CONTRASTS -eq 1 ]; then
#			tot_cont=2*$NUM_COVS*$NUM_GROUPS			# 2 x M cov for 
#		else
#			if [ $DOUBLE_MEAN_CONTRASTS -eq 1 ]; then
#				# $NUM_GROUPS > 1, $NUM_COVS > 0, $DOUBLE_MEAN_CONTRASTS = 1
#				tot_cont=2*$NUM_COVS*$NUM_GROUPS + 2*$NUM_GROUPS 			# 2 x M cov + 2 x G
#			else
#				# $NUM_GROUPS > 1, $NUM_COVS > 0, $DOUBLE_MEAN_CONTRASTS = 0
#				tot_cont=2*$NUM_COVS*$NUM_GROUPS + 1*$NUM_GROUPS 			# 2 x M cov + G
#			fi		
#		fi
#	else # $NUM_COVS = 0
...
#		echo @@@@@@@@@@@@@@@@@@@@@@  $contr_id ----- $NUM_GROUPS ------ $NUM_MEAN_CONTRASTS 
...
#		echo @@@@@@@@@@@@@@@@@@@@@@  $contr_id		
	...
	if [ $NUM_COVS -gt 0 ]; then  # test only interaction covariate x group, no group mean & group comparisons
		...
		do
			...
			ev_id=$NUM_GROUPS+$NUM_NUIS+$cov_id*$NUM_GROUPS+1
			ev_id_plus1=$ev_id+1
					echo @@@@@@@@@@@@@@@@@@@@ $ev_id  -  $ev_id_plus1				
				
			# test correlations differences between groups
			if [ $NUM_GROUPS -eq 2 ]; then
				contr_id=$contr_id+1
				echo "set fmri(conpic_real.$contr_id) 1" >> $OUTPUT_GLM_FSF
				echo "set fmri(conname_real.$contr_id) \"group1 > group2 $COV_LABEL\"" >> $OUTPUT_GLM_FSF
				echo "set fmri(con_real$contr_id.$ev_id) 1" >> $OUTPUT_GLM_FSF				
				echo "set fmri(con_real$contr_id.$ev_id_plus1) -1" >> $OUTPUT_GLM_FSF				

				contr_id=$contr_id+1
				echo "set fmri(conpic_real.$contr_id) 1" >> $OUTPUT_GLM_FSF
				echo "set fmri(conname_real.$contr_id) \"group2 > group1 $COV_LABEL\"" >> $OUTPUT_GLM_FSF
				echo "set fmri(con_real$contr_id.$ev_id) -1" >> $OUTPUT_GLM_FSF				
				echo "set fmri(con_real$contr_id.$ev_id_plus1) 1" >> $OUTPUT_GLM_FSF				
			fi
		done

	else # $NUM_COVS = 0, MEAN_CONTRASTS = 1 or 2
...
#if [ $MEAN_CONTRASTS -gt 0 ]; then  
#	# NO COVARIATES, just group means & comparisons
#	if [ $NUM_GROUPS -eq 1 ]; then
#		echo "set fmri(conpic_real.1) 1" >> $OUTPUT_GLM_FSF
#		echo "set fmri(conname_real.1) \"group1 pos\"" >> $OUTPUT_GLM_FSF
#		echo "set fmri(con_real1.1) 1" >> $OUTPUT_GLM_FSF	
#		
#		if [ $MEAN_CONTRASTS -eq 2 ]; then
#			echo "set fmri(conpic_real.2) 1" >> $OUTPUT_GLM_FSF
#			echo "set fmri(conname_real.2) \"group1 neg\"" >> $OUTPUT_GLM_FSF
#			echo "set fmri(con_real2.1) -1" >> $OUTPUT_GLM_FSF	
#		fi
#	else # NUM_GROUPS > 1 :  $MEAN_CONTRASTS group mean(s) for each group
...
#else
#	# COVARIATES
#	declare -i cov_col_id=0
#	declare -i covid=0
#	if [ $NUM_GROUPS -eq 1 ]; then
#		# :: COVARIATES , 1 GROUP
#		echo "set fmri(conpic_real.1) 1" >> $OUTPUT_GLM_FSF
#		echo "set fmri(conname_real.1) \"group1\"" >> $OUTPUT_GLM_FSF
#		echo "set fmri(con_real1.1) 1" >> $OUTPUT_GLM_FSF	
#	
#		for (( cov_id=0; cov_id<$NUM_COVS; cov_id++ ))
#		do
#			COV_LABEL=${ARR_USED_COV_LABELS[cov_id]}
#			ev=$cov_id+$NUM_NUIS+2
#			
#			# ===== POS CONTRAST	
#			covid=2*cov_id+1
#			echo "set fmri(conpic_real.$covid) 1" >> $OUTPUT_GLM_FSF
#			echo "set fmri(conname_real.$covid) \"pos $COV_LABEL\"" >> $OUTPUT_GLM_FSF
#			echo "set fmri(con_real$covid.$ev) 1" >> $OUTPUT_GLM_FSF
#		
#			# ===== NEG CONTRAST
#			covid=2*cov_id+2
#			echo "set fmri(conpic_real.$covid) 1" >> $OUTPUT_GLM_FSF
#			echo "set fmri(conname_real.$cov_id) \"neg $COV_LABEL\"" >> $OUTPUT_GLM_FSF
#			echo "set fmri(con_real$cov_id.$ev) -1" >> $OUTPUT_GLM_FSF	
#		done		
#	else		# :: COVARIATES , MORE GROUPS
#		
#		for (( cov_id=0; cov_id<$NUM_COVS; cov_id++ ))
#		do
#			COV_LABEL=${ARR_USED_COV_LABELS[cov_id]}
#			for (( grp=1; grp <= $NUM_GROUPS; grp++ ))
#			do	
#				ev=$NUM_GROUPS+$NUM_NUIS+$cov_id*$NUM_GROUPS+$grp		
#			
#				# ===== POS CONTRAST	
#				covid=$(echo "scale=0;2*$cov_id*$NUM_GROUPS+($grp-1)*$NUM_GROUPS+1" | bc)						
#				echo "set fmri(conpic_real.$covid) 1" >> $OUTPUT_GLM_FSF
#				echo "set fmri(conname_real.$covid) \"group$grp pos $COV_LABEL\"" >> $OUTPUT_GLM_FSF
#				echo "set fmri(con_real$covid.$ev) 1" >> $OUTPUT_GLM_FSF
#		
#				# ===== NEG CONTRAST
#				covid=$(echo "scale=0;2*$cov_id*$NUM_GROUPS+($grp-1)*$NUM_GROUPS+2" | bc)						
#				echo "set fmri(conpic_real.$covid) 1" >> $OUTPUT_GLM_FSF
#				echo "set fmri(conname_real.$covid) \"group$grp neg $COV_LABEL\"" >> $OUTPUT_GLM_FSF
#				echo "set fmri(con_real$covid.$ev) -1" >> $OUTPUT_GLM_FSF
#			done
#		done
#	fi
