...
if [ -d "$DTI_DIR" ]; then
  if [ "$(ls -A $DTI_DIR)" ]; then
    ...
    mkdir -p $BEDPOST_DIR
    cp $DTI_DIR/$DTI_EC_IMAGE_LABEL.nii.gz $BEDPOST_DIR/data.nii.gz
    cp $DTI_DIR/$DTI_BVAL $BEDPOST_DIR/bvals
    cp $DTI_DIR/$DTI_ROTATED_BVEC $BEDPOST_DIR/bvecs
    ...
    $FSLDIR/bin/bedpostx $BEDPOST_DIR -n 2 -w 1 -b 1000
