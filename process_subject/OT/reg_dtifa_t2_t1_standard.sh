...
main()
{
	...
	#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	echo "$SUBJ_NAME:						STARTED : nonlin dtifa-t2-t1-standard coregistration"
	# dti -> t2 lin
	[ ! -f $ROI_DIR/reg_t2/dti2t2.mat ] && $FSLDIR/bin/flirt -in $DTI_DIR/dti_FA -ref $T2_BRAIN_DATA -out $ROI_DIR/reg_t2/dti2t2_l -omat $ROI_DIR/reg_t2/dti2t2.mat -bins 256 -cost normmi -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 6 -interp trilinear
	# t2 -> dti lin
	[ ! -f $ROI_DIR/reg_dti/t22dti.mat ] && $FSLDIR/bin/convert_xfm -omat $ROI_DIR/reg_dti/t22dti.mat -inverse $ROI_DIR/reg_t2/dti2t2.mat
	# dti -> t2 nlin
	[ ! -f $ROI_DIR/reg_t2/dti2t2_warp.nii.gz ] && $FSLDIR/bin/fnirt --in=$DTI_DIR/dti_FA --ref=$T2_BRAIN_DATA --aff=$ROI_DIR/reg_t2/dti2t2.mat --cout=$ROI_DIR/reg_t2/dti2t2_warp --iout=$ROI_DIR/reg_t2/nodif_brain2t2_nl  -v &>$ROI_DIR/reg_t2/dti2t2_nl.txt
	# t2 -> dti nlin
	[ ! -f $ROI_DIR/reg_dti/t22dti_warp.nii.gz ] && $FSLDIR/bin/invwarp -r $DTI_DIR/dti_FA -w $ROI_DIR/reg_t2/dti2t2_warp -o $ROI_DIR/reg_dti/t22dti_warp
	#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	# t2 -> highres lin
	...
	# highres -> t2 -> dti
	[ ! -f $ROI_DIR/reg_dti/highres2dti_warp.nii.gz ] && $FSLDIR/bin/invwarp -r $DTI_DIR/dti_FA -w $ROI_DIR/reg_t1/dti2highres_warp -o $ROI_DIR/reg_dti/highres2dti_warp --force
	#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	echo "$SUBJ_NAME:						TERMINATED : nonlin dtifa-t2-t1-standard coregistration"
}
main
