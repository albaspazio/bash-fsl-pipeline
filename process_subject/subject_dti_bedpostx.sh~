...
if [ -d "$DTI_DIR" ]; then
  if [ "$(ls -A $DTI_DIR)" ]; then
    run echo "setting output dir to: $BEDPOST_DIR"

    mkdir -p $BEDPOST_DIR
    run cp $DTI_DIR/$DTI_EC_IMAGE_LABEL.nii.gz $BEDPOST_DIR/data.nii.gz
    run cp $DTI_DIR/$DTI_BVAL $BEDPOST_DIR/bvals
    run cp $DTI_DIR/$DTI_ROTATED_BVEC $BEDPOST_DIR/bvecs
    run cp $DTI_DIR/nodif_brain_mask.nii.gz $BEDPOST_DIR
 
    run $FSLDIR/bin/bedpostx_datacheck $BEDPOST_DIR

    if [ $? -gt 0 ]; then
      run echo "......................................................Error in $BEDPOST_DIR directory"
      exit
    fi
    run $FSLDIR/bin/bedpostx $BEDPOST_DIR -n 2 -w 1 -b 1000
    if [ -f $BEDPOST_OUT_DIR/mean_s0samples.nii.gz ]; then 
    	run mv $BEDPOST_OUT_DIR $DTI_DIR/$OUTPUT_DIR_NAME
    	run rm -rf $BEDPOST_DIR
    else	
    	run echo "ERROR in bedpostx !!!!!"
    fi
 fi
fi
