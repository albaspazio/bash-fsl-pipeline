%% function fslnets_prepare_data_longitudinal(group_maps, tr_value, variance_norm)
...
%%  
function ts = fslnets_prepare_data_longitudinal(group_maps, ts_dir, Nsessions, tr_value, variance_norm, good_nodes, show_graphs)

    %%% load timeseries data from the dual regression output directory
    ts          = nets_load_longitudinal(ts_dir,tr_value,variance_norm, Nsessions);
    if show_graphs
        for s=1:Nsessions
            ts_spectra  = nets_spectra(ts(s));   % have a look at mean timeseries spectra
        end
    end
    ...
    for s=1:Nsessions
        ...
        ts(s)          = nets_tsclean(ts(s),1);   % regress the bad nodes out of the good, and then remove the bad nodes' timeseries (1=aggressive, 0=unaggressive (just delete bad)).
        ...
        if show_graphs
            nets_nodepics(ts(s),group_maps);       % quick views of the good and bad components
            ts_spectra  = nets_spectra(ts(s));     % have a look at mean spectra after this cleanup
        end
    end
end
