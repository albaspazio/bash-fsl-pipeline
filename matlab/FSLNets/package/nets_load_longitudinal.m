...
function [ts] = nets_load_longitudinal(indir,tr,varnorm,Nsessions, varargin);
...
d=dir('*.txt');
Nsubjects=size(d,1)/Nsessions;
for s=1:Nsessions
    TS=[];
    for i=1:Nsubjects
      grotALL=load(d(i+Nsubjects*(s-1)).name);  gn=size(grotALL,1);

      if i==1
        ts(s).NtimepointsPerSubject=gn;
      elseif gn ~= ts(s).NtimepointsPerSubject
    ...
    ts(s).ts=TS;
    ts(s).tr=tr;
    ts(s).Nsessions=Nsessions;
    ts(s).Nsubjects=Nsubjects*Nruns;
    ts(s).Nnodes=size(TS,2);
    ts(s).NnodesOrig=ts(s).Nnodes;
    ts(s).Ntimepoints=size(TS,1);
    ts(s).NtimepointsPerSubject=ts(s).NtimepointsPerSubject/Nruns;
    ts(s).DD=1:ts(s).Nnodes;
    ts(s).UNK=[];
