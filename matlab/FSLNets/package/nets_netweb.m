...
function nets_netweb(netF,netP,DD,sumpics,netwebdir)
    ...
    grot        = prctile(abs(netF(:)),99); 
    netmatL     = netF/grot; 
    netmatH     = netP/grot;
    usenet      = netmatL;  
    usenet(usenet<0)=0;
    N           = size(netmatL,1);  
    grot        = prctile(abs(usenet(:)),99); 
    usenet      = max(min(usenet/grot,1),-1)/2;
    for J = 1:N
        for I = 1:J-1
            y((I-1)*(N-I/2)+J-I) = 0.5 - usenet(I,J);  
        end; 
    end;
    linkages    = linkage(y,'ward');
    set(0,'DefaultFigureVisible','off');
    figure;
    [a,b,hier]  = dendrogram(linkages, 0, 'colorthreshold',0.75);
    close;
