...
function [dpRSN,yyRSN] = nets_hierarchy(netmatL,netmatH,DD,sumpics,rsn_labels,varargin);    %%%% hierarchical clustering figure
...
figure('position',[10 10 1000 500]);  clear y;
N=size(netmatL,1);  gap=.5/(N+1);  
...
subplot('position',[0 H2 1 1-H2]);
  ...
  [dh,dt,dpRSN]=dendrogram(yyRSN,0,'colorthreshold',colour_threshold, 'labels', rsn_labels); set(gca,'ytick',[],'TickLength',[0,0]);
  if exist('octave_config_info')==0
    set(dh,'LineWidth',3);
  end;
