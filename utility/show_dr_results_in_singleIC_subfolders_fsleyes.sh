...
parse_image ()
{
	...
	if [ -n "$THRESH_VALUE" -a -n "$max" ]; then
		...
		if [ "$result" -eq 1 ]; then
			IMAGES_LIST="$IMAGES_LIST $image -cr $THRESH_VALUE 1 -cm red-yellow"
			echo "--------------------------------------------------------------------------------------" >> $result_file
...
if [[ $IMAGES_LIST != "" ]]; then
  if [ $SHOW_IMAGE -eq 1 ]; then
    ...
    if [ -f $FSLDIR/bin/fsleyes ]; then $FSLDIR/bin/fsleyes -ad $BACKGROUND_IMAGE $IMAGES_LIST & 
	else  fsleyes $BACKGROUND_IMAGE $IMAGES_LIST & 
	fi

    echo "--------------------------------------------------------------------------------------" >> $result_file
    echo "fsleyes $BACKGROUND_IMAGE $BACKGROUND_IMAGE2 $IMAGES_LIST &" >> $result_file
    echo "--------------------------------------------------------------------------------------" >> $result_file
  fi
else
  rm $result_file
